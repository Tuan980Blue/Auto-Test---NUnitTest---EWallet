...
{
    public static void WriteTestResult(string filePath, string num1, string num2, string expected, string actual,
        string status)
    {
        if (FileUtil.IsFileInUse(filePath))
        {
            throw new Exception(
                "⚠️ File Excel đang được sử dụng bởi một tiến trình khác. Vui lòng đóng file Excel và thử lại.");
        }
        ...
        using (var package = new ExcelPackage(fileInfo))
        {
            ...
            {
                if (worksheet.Cells[row, 1].Text == num1 && worksheet.Cells[row, 2].Text == num2 &&
                    worksheet.Cells[row, 3].Text == expected)
                {
